...
- (void)setupUI {
  ...
  NSString *mmUrl = [defaults objectForKey:@"MM_mmWebUrl"];
  ...
  if (mmUrl.length > 0) {
    if([mmUrl rangeOfString:@"http"].location == NSNotFound){
      ...
      if (self.mmUrl.length > 0) {
          if([self.mmUrl rangeOfString:@"http"].location == NSNotFound){
              addHttpUrl = [NSString stringWithFormat:@"http://%@", self.mmUrl];
          } else {
              addHttpUrl = self.mmUrl;
          }
      } else {
...
- (void)requestBoxData {
  ...
  NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
  NSString *requestUrl = [defaults objectForKey:@"MM_mmWebUrl"];
